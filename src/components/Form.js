...
export default class Form extends Component {
  ...
  saveData = async () => {
    ...
    if (this.props.type !== 'Login') {
      ...
      Keyboard.dismiss();
      Alert.alert('Mensaje',`Te has registrado con éxito! Correo: ${email}, Contraseña: ${password}`);
      this.login();
    } /* - if - */
      ...
      try {
        ...
        if (ld.email != null && ld.password != null) {
          if (ld.email == email && ld.password == password) {
            Alert.alert('Mensaje','Podemos entrar!');
          } else {
            Alert.alert('Mensaje','La cuenta y/o contraseña no existe!');
          }
        } /* - if - */
      } catch (error) {
        alert(error);
